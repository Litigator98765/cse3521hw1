...
def heappop(openset):
    # thin wrapper over heapq.heappop, allows for random tiebreaking
    mylist = list(heapq.heappop(openset))
    mylist.pop(1)
    return tuple(mylist)

def heappush(openset,seq_to_insert):
    # thin wrapper over heapq.heappush, allows for random tiebreaking
    mylist=list(seq_to_insert)
    mylist.insert(1,random.random())
    heapq.heappush(openset,mylist)